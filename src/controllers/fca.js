...
export const getFcaRepAll = async (req, res) => {
  /* Getting the connection to the database. */
  const pool = await getConection();
...
export const getFcaVta = async (req, res) => {
  /* Getting the connection to the database. */
  const pool = await getConection();
  let {
    initialMonth,
  ...
  WHERE
    ...
    const fcaDev = await pool.request().query(`SELECT
    h.fecha AS FechaFactura,
    h.tipo + '-' + CONVERT(varchar(15), h.numero) AS Factura,
    d.fecha AS FechaPedido,
    d.numero AS NumeroPedido,
  ...
  WHERE
    ...
    if (!!fca) {
      const sumVta = fca.recordset.reduce(
      ...
      const sumDev = fcaDev.recordset.reduce(
        (acc, row) => acc + row.ventatotal,
        0
      );
      const pricedRepsDev = fcaDev.recordset.map((rep) => {
        return {
          ...rep,
          ventatotal: formatCOP(parseInt(rep.ventatotal)),
        };
...
export const getFcaCarter = async (req, res) => {
  ...
  try {
    ...
    const fca = await pool.request().query(`select * from CARTER01_2021_FCA`);
...
export const getFcaVentastaller = async (req, res) => {
  /* Getting the connection to the database. */
  const pool = await getConection();
  ...
  try {
    ...
    const fcaMos = await pool.request()
  ...
  WHERE (z.nit <> 900531238) AND (z.tipo = 'FVMM') AND (z.fecha BETWEEN '${initialDate
    .toISOString()
    .slice(0, 10)} 00:00:00.000' AND '${finalDate
      .toISOString()
      .slice(0, 10)} 23:59:59.999')`);

    const fcaMosDev = await pool.request()
      .query(`SELECT CONVERT(varchar, z.fecha, 103) AS FechaFactura, 
      z.tipo + '-' + CONVERT(varchar(15), z.numero) AS Factura, 
      CASE WHEN d .condicion = 01 THEN 'contado' ELSE 'Credito' END AS TipoPago, 
      z.descuento AS Descuento, 
  ...
  FROM            dbo.z_vta_repuestos AS z LEFT OUTER JOIN
      ...
      dbo.terceros_nombres AS tn ON te.nit = tn.nit LEFT OUTER JOIN
      dbo.Crmv_terceros_medio_contacto AS cr ON te.id = cr.IdTerceros
  WHERE (z.nit <> 900531238) AND (z.tipo = 'DFMM') AND (z.fecha BETWEEN '${initialDate
    .toISOString()
    .slice(0, 10)} 00:00:00.000' AND '${finalDate
      .toISOString()
      .slice(0, 10)} 23:59:59.999')`);

    const fcaTall = await pool.request()
  ...
  WHERE   (v.bodega IN (15)) 
    ...
    const fcaTallDev = await pool.request()
      .query(`SELECT  CONVERT(varchar, v.fecha_facturacion, 103) AS FechaFactura, 
      LTRIM(RTRIM(v.tipo_num_fac)) AS Factura,  
      ISNULL(cp.descripcion, 'CONTADO') AS TipoPago,
     (
  ...
  WHERE   (v.bodega IN (15)) 
    ...
    if (!!fcaMos) {
      const fixedMos = fcaMos.recordset.map((obj) => {
        const cliente =
          obj.NombreCliente || obj.RazonSocial || "No se encontraron datos";
        const telefono =
          obj.Telefono || obj.Celular || "No se encontraron datos";
      ...
      const fixedMosDev = fcaMosDev.recordset.map((obj) => {
        const cliente =
          obj.NombreCliente || obj.RazonSocial || "No se encontraron datos";
        const telefono =
          obj.Telefono || obj.Celular || "No se encontraron datos";
      ...
      const fixedTall = fcaTall.recordset.map((obj) => {
        const cliente =
          obj.NombreCliente || obj.RazonSocial || "No se encontraron datos";
        const telefono =
      ...
      const fixedTallDev = fcaTallDev.recordset.map((obj) => {
        const cliente =
          obj.NombreCliente || obj.RazonSocial || "No se encontraron datos";
        const telefono =
          obj.Telefono1 || obj.Celular1 || obj.Telefono2 || obj.Celular2 ||"No se encontraron datos";
