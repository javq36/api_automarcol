...
from
    ...
    if (!!ford) {
      const filteredCars = ford.recordset.reduce((uniqueCars, car) => {
...
export const getFordrepAll = async (req, res) => {
  /* Getting the connection to the database. */
  const pool = await getConection();
...
export const getFordVta = async (req, res) => {
  /* Getting the connection to the database. */
  const pool = await getConection();
  let {
    initialMonth,
  ...
  WHERE
    ...
    const fordDev = await pool.request().query(`SELECT
    h.fecha AS FechaFactura,
    h.tipo + '-' + CONVERT(varchar(15), h.numero) AS Factura,
    d.numero AS NumeroPedido,
    b.serie AS Vin,
  ...
  WHERE
    ...
    if (!!ford) {
      const sumVta = ford.recordset.reduce(
      ...
      const sumDev = fordDev.recordset.reduce(
        (acc, row) => acc + row.ventatotal,
        0
      );
      ...
      const resultldev = fixModelDev.map((obj) => {
        const cliente =
          obj.NombreCliente || obj.RazonSocial || "No se encontraron datos";
        const telefono =
          obj.Telefono || obj.Celular || "No se encontraron datos";
      ...
      return res
        .status(200)
        .json([result, resultldev, sumVta, sumDev, gananciaNeta]);
    }
    /*  if everything else fails, return a 404 error. */
    return res.status(404).json({ message: "operation failed" });
  } catch (error) {
...
export const getFordVentastaller = async (req, res) => {
  /* Getting the connection to the database. */
  const pool = await getConection();
  ...
  try {
    ...
    const fordMos = await pool.request()
  ...
  WHERE (z.nit <> 900531238) AND (z.tipo = 'FVM') AND (z.fecha BETWEEN '${initialDate
    .toISOString()
    .slice(0, 10)} 00:00:00.000' AND '${finalDate
      .toISOString()
      .slice(0, 10)} 23:59:59.999')`);

    const fordMosDev = await pool.request()
      .query(`SELECT CONVERT(varchar, z.fecha, 103) AS FechaFactura, 
      z.tipo + '-' + CONVERT(varchar(15), z.numero) AS Factura, 
      CASE WHEN d .condicion = 01 THEN 'contado' ELSE 'Credito' END AS TipoPago, 
      z.descuento AS Descuento, 
  ...
  FROM            dbo.z_vta_repuestos AS z LEFT OUTER JOIN
      ...
      dbo.terceros_nombres AS tn ON te.nit = tn.nit LEFT OUTER JOIN
      dbo.Crmv_terceros_medio_contacto AS cr ON te.id = cr.IdTerceros
  WHERE (z.nit <> 900531238) AND (z.tipo = 'DFVM') AND (z.fecha BETWEEN '${initialDate
    .toISOString()
    .slice(0, 10)} 00:00:00.000' AND '${finalDate
      .toISOString()
      .slice(0, 10)} 23:59:59.999')`);

    const fordTall = await pool.request()
  ...
  WHERE   (v.bodega IN (5)) 
    ...
    const fordTallDev = await pool.request()
      .query(`SELECT  CONVERT(varchar, v.fecha_facturacion, 103) AS FechaFactura, 
      LTRIM(RTRIM(v.tipo_num_fac)) AS Factura,  
      ISNULL(cp.descripcion, 'CONTADO') AS TipoPago,
     (
  ...
  WHERE   (v.bodega IN (5)) 
    ...
    if (!!fordMos) {
      const fixedMos = fordMos.recordset.map((obj) => {
        const cliente =
          obj.NombreCliente || obj.RazonSocial || "No se encontraron datos";
        const telefono =
          obj.Telefono || obj.Celular || "No se encontraron datos";
      ...
      const fixedMosDev = fordMosDev.recordset.map((obj) => {
        const cliente =
          obj.NombreCliente || obj.RazonSocial || "No se encontraron datos";
        const telefono =
          obj.Telefono || obj.Celular || "No se encontraron datos";
      ...
      const fixedTall = fordTall.recordset.map((obj) => {
        const cliente =
          obj.NombreCliente || obj.RazonSocial || "No se encontraron datos";
        const telefono =
      ...
      const fixedTallDev = fordTallDev.recordset.map((obj) => {
        const cliente =
          obj.NombreCliente || obj.RazonSocial || "No se encontraron datos";
        const telefono =
          obj.Telefono1 || obj.Celular1 || obj.Telefono2 || obj.Celular2 ||"No se encontraron datos";
