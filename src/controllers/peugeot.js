...
from
    ...
    LEFT JOIN VTANUE_PEUGEOT_HISTORICO as vt on vt.Version_DescipcionModelo = I1.Version_DescipcionModelo
    LEFT JOIN img_modelo as img on img.modelo = I1.Version_DescipcionModelo`);

    if (!!peugeot) {
      const filteredCars = peugeot.recordset.reduce((uniqueCars, car) => {
...
export const getPeugeotRep = async (req, res) => {
  /* Getting the connection to the database. */
  const pool = await getConection();
  ...
  try {
    ...
    const peugeot = await pool.request().query(`select distinct 
    I1.Bodega AS Marca,
...
export const getPeugeotRepAll = async (req, res) => {
  /* Getting the connection to the database. */
  const pool = await getConection();
...
export const getPeugeotVta = async (req, res) => {
  /* Getting the connection to the database. */
  const pool = await getConection();
  let {
    initialMonth,
  ...
  WHERE
    ...
    const peugeotDev = await pool.request().query(`SELECT
    h.fecha AS FechaFactura,
    h.tipo + '-' + CONVERT(varchar(15), h.numero) AS Factura,
    d.fecha AS FechaPedido,
    d.numero AS NumeroPedido,
  ...
  WHERE
    ...
    if (!!peugeot) {
      const sumVta = peugeot.recordset.reduce(
      ...
      const sumDev = peugeotDev.recordset.reduce(
        (acc, row) => acc + row.ventatotal,
        0
      );
      const pricedRepsDev = peugeotDev.recordset.map((rep) => {
        return {
          ...rep,
          ventatotal: formatCOP(parseInt(rep.ventatotal)),
        };
...
export const getPeugeotCarter = async (req, res) => {
  /* Getting the connection to the database. */
  const pool = await getConection();
  ...
  try {
    ...
    const peugeot = await pool
      .request()
      .query(`select * from CARTER01_2021_PEUGEOT`);
...
export const getPeugeotVentastaller = async (req, res) => {
  /* Getting the connection to the database. */
  const pool = await getConection();
  ...
  try {
    ...
    const peugeotMos = await pool.request()
  ...
  WHERE (z.nit <> 900531238) AND (z.tipo = 'FVMP') AND (z.fecha BETWEEN '${initialDate
    .toISOString()
    .slice(0, 10)} 00:00:00.000' AND '${finalDate
      .toISOString()
      .slice(0, 10)} 23:59:59.999')`);

    const peugeotMosDev = await pool.request()
      .query(`SELECT CONVERT(varchar, z.fecha, 103) AS FechaFactura, 
      z.tipo + '-' + CONVERT(varchar(15), z.numero) AS Factura, 
      CASE WHEN d .condicion = 01 THEN 'contado' ELSE 'Credito' END AS TipoPago, 
      z.descuento AS Descuento, 
  ...
  FROM            dbo.z_vta_repuestos AS z LEFT OUTER JOIN
      ...
      dbo.terceros_nombres AS tn ON te.nit = tn.nit LEFT OUTER JOIN
      dbo.Crmv_terceros_medio_contacto AS cr ON te.id = cr.IdTerceros
  WHERE (z.nit <> 900531238) AND (z.tipo = 'DFMP') AND (z.fecha BETWEEN '${initialDate
    .toISOString()
    .slice(0, 10)} 00:00:00.000' AND '${finalDate
      .toISOString()
      .slice(0, 10)} 23:59:59.999')`);

    const peugeotTall = await pool.request()
  ...
  WHERE   (v.bodega IN (18)) 
    ...
    const peugeotTallDev = await pool.request()
      .query(`SELECT  CONVERT(varchar, v.fecha_facturacion, 103) AS FechaFactura, 
      LTRIM(RTRIM(v.tipo_num_fac)) AS Factura,  
      ISNULL(cp.descripcion, 'CONTADO') AS TipoPago,
     (
  ...
  WHERE   (v.bodega IN (18)) 
    ...
    if (!!peugeotMos) {
      const fixedMos = peugeotMos.recordset.map((obj) => {
        const cliente =
          obj.NombreCliente || obj.RazonSocial || "No se encontraron datos";
        const telefono =
          obj.Telefono || obj.Celular || "No se encontraron datos";
      ...
      const fixedMosDev = peugeotMosDev.recordset.map((obj) => {
        const cliente =
          obj.NombreCliente || obj.RazonSocial || "No se encontraron datos";
        const telefono =
          obj.Telefono || obj.Celular || "No se encontraron datos";
      ...
      const fixedTall = peugeotTall.recordset.map((obj) => {
        const cliente =
          obj.NombreCliente || obj.RazonSocial || "No se encontraron datos";
        const telefono =
      ...
      const fixedTallDev = peugeotTallDev.recordset.map((obj) => {
        const cliente =
          obj.NombreCliente || obj.RazonSocial || "No se encontraron datos";
        const telefono =
          obj.Telefono1 || obj.Celular1 || obj.Telefono2 || obj.Celular2 ||"No se encontraron datos";
