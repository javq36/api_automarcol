...
export const getBajajrepAll = async (req, res) => {
  /* Getting the connection to the database. */
  const pool = await getConection();
...
export const getBajajVta = async (req, res) => {
  ...
  WHERE
    ...
    const bajajDev = await pool.request().query(`SELECT
    h.fecha AS FechaFactura,
    h.tipo + '-' + CONVERT(varchar(15), h.numero) AS Factura,
    d.fecha AS FechaPedido,
    d.numero AS NumeroPedido,
  ...
  WHERE
    ...
    if (!!bajaj) {
      const sumVta = bajaj.recordset.reduce(
      ...
      const sumDev = bajajDev.recordset.reduce(
        (acc, row) => acc + row.ventatotal,
        0
      );
      const pricedRepsDev = bajajDev.recordset.map((rep) => {
        return {
          ...rep,
          ventatotal: formatCOP(parseInt(rep.ventatotal)),
        };
      ...
      let gananciaNeta = sumDev + sumDev;
      ...
      return res
        .status(200)
        .json([result, resultDev, sumVta, sumDev, gananciaNeta]);
    }
    /*  if everything else fails, return a 404 error. */
    return res.status(404).json({ message: "operation failed" });
  } catch (error) {
...
export const getBajajCarter = async (req, res) => {
  /* Getting the connection to the database. */
  const pool = await getConection();
  ...
  try {
    ...
    const bajaj = await pool
      .request()
      .query(`select * from CARTER01_2021_BAJAJ`);
...
export const getBajajVentastaller = async (req, res) => {
  /* Getting the connection to the database. */
  const pool = await getConection();
  ...
  try {
    ...
    const bajajMos = await pool.request()
  ...
  WHERE (z.nit <> 900531238) AND (z.tipo = 'FVMB') AND (z.fecha BETWEEN '${initialDate
    .toISOString()
    .slice(0, 10)} 00:00:00.000' AND '${finalDate
      .toISOString()
      .slice(0, 10)} 23:59:59.999')`);

    const bajajMosDev = await pool.request()
      .query(`SELECT CONVERT(varchar, z.fecha, 103) AS FechaFactura, 
      z.tipo + '-' + CONVERT(varchar(15), z.numero) AS Factura, 
      CASE WHEN d .condicion = 01 THEN 'contado' ELSE 'Credito' END AS TipoPago, 
      z.descuento AS Descuento, 
  ...
  FROM            dbo.z_vta_repuestos AS z LEFT OUTER JOIN
      ...
      dbo.terceros_nombres AS tn ON te.nit = tn.nit LEFT OUTER JOIN
      dbo.Crmv_terceros_medio_contacto AS cr ON te.id = cr.IdTerceros
  WHERE (z.nit <> 900531238) AND (z.tipo = 'DFMB') AND (z.fecha BETWEEN '${initialDate
    .toISOString()
    .slice(0, 10)} 00:00:00.000' AND '${finalDate
      .toISOString()
      .slice(0, 10)} 23:59:59.999')`);

    const bajajTall = await pool.request()
  ...
  WHERE   (v.bodega IN (21)) 
    ...
    const bajajTallDev = await pool.request()
      .query(`SELECT  CONVERT(varchar, v.fecha_facturacion, 103) AS FechaFactura, 
      LTRIM(RTRIM(v.tipo_num_fac)) AS Factura,  
      ISNULL(cp.descripcion, 'CONTADO') AS TipoPago,
     (
  ...
  WHERE   (v.bodega IN (21)) 
    ...
    if (!!bajajMos) {
      const fixedMos = bajajMos.recordset.map((obj) => {
        const cliente =
          obj.NombreCliente || obj.RazonSocial || "No se encontraron datos";
        const telefono =
          obj.Telefono || obj.Celular || "No se encontraron datos";
      ...
      const fixedMosDev = bajajMosDev.recordset.map((obj) => {
        const cliente =
          obj.NombreCliente || obj.RazonSocial || "No se encontraron datos";
        const telefono =
          obj.Telefono || obj.Celular || "No se encontraron datos";
      ...
      const fixedTall = bajajTall.recordset.map((obj) => {
        const cliente =
          obj.NombreCliente || obj.RazonSocial || "No se encontraron datos";
        const telefono =
      ...
      const fixedTallDev = bajajTallDev.recordset.map((obj) => {
        const cliente =
          obj.NombreCliente || obj.RazonSocial || "No se encontraron datos";
        const telefono =
          obj.Telefono1 || obj.Celular1 || obj.Telefono2 || obj.Celular2 ||"No se encontraron datos";
